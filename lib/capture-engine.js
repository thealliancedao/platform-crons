...
// ── Shared data sources ──────────────────────────────────────────────────────
// ORG (2026-08-11): the tla-snapshot producer was folded into member-data;
// the legacy repo's copy is frozen and the repo is slated for deletion.
const TLA_SNAPSHOT_URL     = 'https://raw.githubusercontent.com/thealliancedao/tla-core/main/member-data/tla-snapshot/current.json';
// ORG (2026-08-11): legacy prices repo is DELETED (404) — this 404 was what
// broke adao-positions + tla-participants. Org nap carries the same fields
// this engine reads (token_prices, lst_ratios), verified 27 tokens.
...
function projectLockVp(lock, symbolToLstRatio) {
    ...
    // SPEC-vp-definition-fix (2026-07-13): all VP here is TOTAL basis =
    // voting_power (boost) + fixed_amount. A restamp raises the underlying,
    // which raises BOTH components: adjusted total = underlying_now × (coeff + 1).
    const assetAmount = parseFloat(lock.asset?.amount) || 0;
    const underlyingAtLock = parseFloat(lock.underlying_amount) || 0;
    const currentVp = (parseFloat(lock.voting_power) || 0) + (parseFloat(lock.fixed_amount) || 0);
    ...
    const newVp = newUnderlying * (coefficient + 1);  // boost (×coeff) + fixed (×1)
...
async function fetchMemberPortfolio(member, ctx) {
    ...
    for (const { bucket, staked } of stakingResults) {
        for (const entry of staked) {
            try {
                ...
                // v1.4.0 (reconciliation 2026-08-24, CAPA supply probe): user_pct_of_pool
                // used user SHARES over the TLA bucket's total SHARES, then multiplied the
                // WHOLE pool's reserves by it — two mismatched denominators. Treasury
                // CAPA-LUNA published 119,157 CAPA underlying; the redeemable claim is
                // amount (18,411.23 LP, post-take) × reserve ÷ lp_health.total_share
                // = 109,462 (8.9% lower). Correct basis: LP over LP.
                //   pct of POOL   = redeemable LP amount ÷ pool total LP supply
                //   pct of BUCKET = shares ÷ bucket total shares (kept, for reward math)
                if (totalShares > 0) {
                    position.user_pct_of_tla_bucket = (shares / totalShares) * 100;
                }
                const poolLpSupply = parseFloat(pool?.lp_health?.total_share) || 0;
                if (poolLpSupply > 0) {
                    position.user_pct_of_pool = (balance / poolLpSupply) * 100;
                    position.pct_basis = 'lp_amount_over_pool_supply';
                } else if (totalShares > 0) {
                    // No lp_health (single-asset bucket): bucket-share basis is the only
                    // denominator available — labeled so readers know which math this is.
                    position.user_pct_of_pool = position.user_pct_of_tla_bucket;
                    position.pct_basis = 'shares_over_bucket_shares';
                }

                if (pool && position.user_pct_of_pool != null) {
                    // USD must use the SAME denominator as the pct: whole-pool pct ×
                    // whole-pool USD (depth_usd). staked_in_tla_usd only pairs with the
                    // bucket-share pct.
                    if (position.pct_basis === 'lp_amount_over_pool_supply' && pool.depth_usd) {
                        position.estimated_position_usd = pool.depth_usd * (position.user_pct_of_pool / 100);
                    } else if (pool.staked_in_tla_usd && position.user_pct_of_tla_bucket != null) {
                        position.estimated_position_usd = pool.staked_in_tla_usd * (position.user_pct_of_tla_bucket / 100);
                        position.usd_basis = 'staked_in_tla_usd_x_bucket_pct';
                    }
                }
    ...
    // SPEC-vp-definition-fix (2026-07-13): total VP = voting_power (boost) +
    // fixed_amount — chain-canonical (escrow total_vamp.vp = fixed + boost);
    // the money (gauge distributions) follows the sum. Verified against the
    // TLA UI to displayed precision (1,179,504.21 + 131,056.04 = 1,310,560.25
    // = UI "1.31M VP").
    {
        const vpBoost = parseFloat(userInfo?.voting_power) || 0;
        const vpFixed = parseFloat(userInfo?.fixed_amount) || 0;
        portfolio.voting = {
            total_voting_power_human: (vpBoost + vpFixed) / 1e6,  // TOTAL (boost + fixed)
            vp_boost_raw: userInfo?.voting_power || '0',
            vp_boost_human: vpBoost / 1e6,
            fixed_amount_raw: userInfo?.fixed_amount || '0',
            fixed_amount_human: vpFixed / 1e6,
        ...
        for (const { tokenId, lockInfo } of lockInfos) {
            ...
            try {
                ...
                portfolio.locks.push({
                    ...
                    voting_power_raw: lockInfo.voting_power,   // boost component
                    voting_power_human: (parseFloat(lockInfo.voting_power) || 0) / 1e6,
                    fixed_amount_raw: lockInfo.fixed_amount,
                    fixed_amount_human: (parseFloat(lockInfo.fixed_amount) || 0) / 1e6,
                    // SPEC-vp-definition-fix: the lock's effective VP (boost + fixed)
                    vp_total_human: ((parseFloat(lockInfo.voting_power) || 0) + (parseFloat(lockInfo.fixed_amount) || 0)) / 1e6,
...
function computeMemberSummary(portfolio, ctx) {
    ...
    return {
        // SPEC-vp-definition-fix (2026-07-13): voting_power_human is the TOTAL
        // (boost + fixed) — the value the TLA UI displays and the value gauge
        // distributions pay against. The former claim here that boost alone
        // "determines vote weights" was FALSE (disproved by contract probes;
        // see the spec). display_voting_power_human (fixed × 10) is RETIRED —
        // it only coincidentally matched for coefficient-9 max locks.
        voting_power_human: portfolio.voting.total_voting_power_human,
        vp_boost_human: portfolio.voting.vp_boost_human,
