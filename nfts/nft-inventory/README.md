# nfts/nft-inventory — the NFT inventory engine (tenant-agnostic)

**Moved from `nfts/adao/` on 2026-09-18 (Rev D.1).** The folder is the ENGINE, not a tenant: it holds no collection's
identity. Which collections a service runs comes from `tla-core/docs/curated/tenants.json` (`ALLY=<slug>` →
`node run-ally.js` → one `node index.js` process per collection with `COLLECTION=<slug>`, each reading
`nft-collections/<slug>/collection.json`). Stop an ally = stop its Render service; nothing here changes.

Services: `org-nft-inventory` (ALLY=adao) · `org-nft-inventory-liondao` (ALLY=liondao) · `org-nft-adao-daily` (flows.js,
the aDAO daily state-diff — aDAO-only until it is folded into the engine).
