...
(async () => {
  ...
  ok(J('pixel-lions/nft-flows/heartbeat.json').version === '1.2.2' && J('pixel-lions/nft-flows/heartbeat.json').status === 'ok', 'heartbeat 1.2.2 ok');
  ...
  { const led4 = J('pixel-lions/ledger/2026/09.json'); const n0 = led4.length;
    ...
    { const led6 = J('pixel-lions/ledger/2026/09.json');
      ...
      BLUNA_DAILY['2024-10-16'] = 0.5; gh.length = 0;   // 1.2.2: the series is SPARSE — the 18th is missing, the 16th exists
      const r6 = await run(); const old = J('pixel-lions/ledger/2024/10.json')[0]; const hb6 = J('pixel-lions/nft-flows/heartbeat.json');
      ok(r6.status === 0 && old.denom_symbol === 'bLUNA' && Math.abs(old.usd - 100) < 1e-9 && old.usd_basis === 'bluna-usd-daily:2024-10-16 (nearest, Δ2d)' && old.usd_repriced_at, '1.2.1/1.2.2 an old month listed in the index is walked: the 2024 bLUNA repair row is priced from the NEAREST day of a sparse series, the basis says which day and the gap', old);
      ...
      ok(r6b.status === 0 && far.usd == null && far.usd_reason === 'usd_daily_missing:bLUNA:2024-10-30:nearest>7d' && far.denom_symbol === 'bLUNA', '1.2.2 a row more than 7 days from any series day stays null — and its reason now says the day and the gap', far);
