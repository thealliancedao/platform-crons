...
function evalRules(cfg, series, mult) {
  ...
  for (let i = 1; i < snaps.length; i++) {
    ...
    if (on('gauge_set_change')) {
      for (const [g, p] of Object.entries(byB)) { const q = byA[g]; if (!q) out.push(row({ date: b.date, rule: 'gauge_set_change', kind: 'added', gauge_pool_id: g, name: p.name, bucket: p.bucket, label: `${p.name || g} joined the ${p.bucket} bucket`, value: p.status || '', raw: { status: p.status } })); else if ((q.status || '') !== (p.status || '')) out.push(row({ date: b.date, rule: 'gauge_set_change', kind: 'status', gauge_pool_id: g, name: p.name, bucket: p.bucket, label: `${p.name || g}: ${q.status || '?'} → ${p.status || '?'}`, value: p.status || '', raw: { from: q.status, to: p.status } })); }
      for (const [g, q] of Object.entries(byA)) if (!byB[g]) out.push(row({ date: b.date, rule: 'gauge_set_change', kind: 'removed', gauge_pool_id: g, name: q.name, bucket: q.bucket, label: `${q.name || g} left the ${q.bucket} bucket`, value: '', raw: { was: q.status } }));
    }
    ...
    if (!sameEpoch && on('epoch_flip')) {
      ...
      out.push(row({ date: b.date, rule: 'epoch_flip', kind: 'epoch', epoch: b.epoch, label: `Epoch ${b.epoch} started — ${Math.round(tot / 1e6 * 10) / 10}M VP across ${moves.length} buckets`, value: moves.map(m => `${m.bucket} ${m.pct == null ? '' : (m.pct >= 0 ? '+' : '') + m.pct.toFixed(1) + '%'}`).join(' · '), raw: { buckets: moves } }));
