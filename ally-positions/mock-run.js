...
global.fetch = async (url) => { url = String(url); calls.push(url);
  ...
  if (url.includes('/cosmos/base/tendermint/v1beta1/blocks/')) { const h = url.endsWith('latest') ? 22950000 : Number(url.split('/blocks/')[1]); return ok({ block: { header: { height: String(h), time: new Date(1758500000000 + (h - 22930000) * 6100).toISOString() } } }); }   // 1.2.0: 6.1 s blocks
  ...
  if (url.includes('/cosmwasm/wasm/v1/contract/')) { const c = url.split('/contract/')[1].split('/')[0]; const q = b64q(url);
    // 1.2.0 SHAPE fixtures: the DAODAO rewards distributor (found by the claim tx 8896AD9B…): two distributions — 1 = ROAR cw20 linear
    // per time (5B ROAR per 365 d), 2 = DROGO native linear per blocks; pending per wallet (treasury has some of both)
    if (c === T.staking.pl_rewards_distributor && q) {
      if (q.distributions) return ok({ data: { distributions: [
        { id: 1, denom: { cw20: ROAR }, active_epoch: { emission_rate: { linear: { amount: '5000000000000000', duration: { time: 31536000 }, continuous: true } }, started_at: { at_time: '1780852663404426773' }, ends_at: { at_time: '1798132674404426773' }, total_earned_puvp: '0' }, vp_contract: T.staking.pl_voting_module, hook_caller: T.staking.pl_voting_module, funded_amount: '4200000000000000', open_funding: true },
        { id: 2, denom: { cw20: 'terra1cl273523kmr2uwjhhznq54je69mted2u3ljffm8kp2ap4z3drdksftwqun' }, active_epoch: { emission_rate: { linear: { amount: '100000000000000', duration: { height: 10000 }, continuous: true } }, started_at: { height: 22900000 }, ends_at: { height: 23900000 }, total_earned_puvp: '0' }, vp_contract: T.staking.pl_voting_module, hook_caller: T.staking.pl_voting_module, funded_amount: '900000000000000', open_funding: true } ] } });
      if (q.pending_rewards) return ok({ data: { pending_rewards: q.pending_rewards.address === treasury ? [{ id: 1, denom: { cw20: ROAR }, pending_rewards: '12345678000000' }, { id: 2, denom: { cw20: 'terra1cl273523kmr2uwjhhznq54je69mted2u3ljffm8kp2ap4z3drdksftwqun' }, pending_rewards: '99000000000' }] : [] } });
    }
...
(async () => {
  ...
  chk('1.1.0 sources block: credia markets (11) + as-of, feed denoms indexed, known_cw20s count', doc.sources && doc.sources.credia_markets === 11 && doc.sources.feed_denoms_indexed > 10 && doc.sources.known_cw20s === 2, doc.sources);
  ...
  // ---- 1.2.0 the pixeLions rewards distributor
  { const R = doc.pl_rewards; const roarPx = 3.21157968177e-07; const staked = 2929;
    chk('1.2.0 pl_rewards: distributor from the registry, two distributions read (raw kept on each), staked count from the summary, block time measured', R && R.distributor === T.staking.pl_rewards_distributor && R.distributions.length === 2 && R.distributions.every(d => d.raw && d.raw.active_epoch) && R.staked_count === staked && Math.abs(R.block_seconds - 6.1) < 0.01, R && [R.distributor, R.staked_count, R.block_seconds]);
    const d1 = R.distributions[0], d2 = R.distributions[1];
    chk('1.2.0 distribution 1 = ROAR, linear per TIME: 5B/yr → per token/yr = 5B ÷ 2,929, priced by the feed, funded 4.2B', d1.symbol === 'ROAR' && Math.abs(d1.emission_per_year - 5e9) < 1 && Math.abs(d1.per_token_per_year - 5e9 / staked) < 1e-6 && Math.abs(d1.price_usd - roarPx) < 1e-15 && Math.abs(d1.per_token_usd_per_year - 5e9 / staked * roarPx) < 1e-9 && d1.funded_amount === 4.2e9, d1);
    chk('1.2.1 distribution 2 = DROGO (the cw20 named by the registry, found by the claim tx), linear per BLOCKS: 100M per 10k blocks at 6.1 s → per year via the measured block time; not in the feed → per_token known, USD null, listed under apr.unpriced by SYMBOL', d2.symbol === 'DROGO' && d2.denom === 'terra1cl273523kmr2uwjhhznq54je69mted2u3ljffm8kp2ap4z3drdksftwqun' && Math.abs(d2.emission_per_year - 1e8 * (31536000 / 6.1) / 10000) < 1 && d2.per_token_per_year != null && d2.per_token_usd_per_year == null && /blocks at 6\.10s/.test(d2.rate_note) && R.apr.unpriced.length === 1 && R.apr.unpriced[0] === 'DROGO', [d2.emission_per_year, d2.rate_note, R.apr.unpriced]);
    chk('1.2.1 epoch dates as ISO from at_time (ns): distribution 1 started 2026-06-07, ends 2026-12-24; continuous flag kept', d1.started_at.startsWith('2026-06-07') && d1.ends_at.startsWith('2026-12-24') && d1.continuous === true, [d1.started_at, d1.ends_at, d1.continuous]);
    const floor = Math.min(...Object.values(J(path.join(NFTC, 'pixel-lions/snapshots/nft-analytics.json')).listings_by_marketplace).map(m => m.floor_usd));
    chk('1.2.0 APR at floor = (ROAR per token/yr × price) ÷ lowest venue floor (nft-analytics) — arithmetic, the source named', Math.abs(R.apr.usd_per_token_per_year - d1.per_token_usd_per_year) < 1e-12 && R.apr.floor_usd === floor && Math.abs(R.apr.apr_pct_at_floor - d1.per_token_usd_per_year / floor * 100) < 1e-9 && /nft-analytics/.test(R.apr.floor_source), R.apr);
    chk('1.2.0 pending per roster wallet: treasury 12.345678M ROAR (priced) + 99k DROGO (unpriced); other wallets an EMPTY list (0), never null', R.pending_by_wallet[treasury].length === 2 && Math.abs(R.pending_by_wallet[treasury][0].amount - 12345678) < 1e-6 && R.pending_by_wallet[treasury][0].usd != null && R.pending_by_wallet[treasury][1].usd == null && R.pending_by_wallet[treasury][1].symbol === 'DROGO' && Object.keys(R.pending_by_wallet).filter(a => a !== treasury).every(a => Array.isArray(R.pending_by_wallet[a]) && R.pending_by_wallet[a].length === 0), R.pending_by_wallet[treasury]); }
  // ---- 1.2.2 the holders duty is due when its heartbeat is missing or ≥ 20 h old; HOLDERS=off disables it
  { const save = global.fetch; const withHb = (hb) => { global.fetch = async (u) => String(u).includes('holders-heartbeat.json') ? (hb ? ok(hb) : nf) : save(u); };
    withHb(null); chk('1.2.2 holdersDue: no heartbeat → due', (await P.holdersDue('lion-dao')).due === true);
