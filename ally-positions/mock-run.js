  ...
  if (url.includes('/cosmwasm/wasm/v1/contract/')) { const c = url.split('/contract/')[1].split('/')[0]; const q = b64q(url);
    ...
    if (c === T.staking.pl_rewards_distributor && q) {
      if (q.distributions) return ok({ data: { distributions: [
        { id: 1, denom: { cw20: ROAR }, active_epoch: { emission_rate: { linear: { amount: '5000000000000000', duration: { time: 31536000 }, continuous: true } }, started_at: { time: '1758000000000000000' }, ends_at: { time: '1789536000000000000' }, total_earned_puvp: '0' }, vp_contract: T.staking.pl_voting_module, hook_caller: T.staking.pl_voting_module, funded_amount: '4200000000000000', open_funding: true },
        { id: 2, denom: { native: 'factory/terra1mockdrogo/DROGO' }, active_epoch: { emission_rate: { linear: { amount: '100000000000000', duration: { height: 10000 }, continuous: true } }, started_at: { height: 22900000 }, ends_at: { height: 23900000 }, total_earned_puvp: '0' }, vp_contract: T.staking.pl_voting_module, hook_caller: T.staking.pl_voting_module, funded_amount: '900000000000000', open_funding: true } ] } });
      if (q.pending_rewards) return ok({ data: { pending_rewards: q.pending_rewards.address === treasury ? [{ id: 1, denom: { cw20: ROAR }, pending_rewards: '12345678000000' }, { id: 2, denom: { native: 'factory/terra1mockdrogo/DROGO' }, pending_rewards: '99000000000' }] : [] } });
    }
...
(async () => {
  ...
  { const R = doc.pl_rewards; const roarPx = 3.21157968177e-07; const staked = 2929;
    ...
    chk('1.2.0 distribution 2 = DROGO, linear per BLOCKS: 100M per 10k blocks at 6.1 s → per year via the measured block time; not in the feed → per_token known, USD null, listed under apr.unpriced', d2.symbol == null && d2.denom === 'factory/terra1mockdrogo/DROGO' && Math.abs(d2.emission_per_year - 1e8 * (31536000 / 6.1) / 10000) < 1 && d2.per_token_per_year != null && d2.per_token_usd_per_year == null && /blocks at 6\.10s/.test(d2.rate_note) && R.apr.unpriced.length === 1 && R.apr.unpriced[0] === 'factory/terra1mockdrogo/DROGO', [d2.emission_per_year, d2.rate_note, R.apr.unpriced]);
    ...
    chk('1.2.0 pending per roster wallet: treasury 12.345678M ROAR (priced) + 99k DROGO (unpriced); other wallets an EMPTY list (0), never null', R.pending_by_wallet[treasury].length === 2 && Math.abs(R.pending_by_wallet[treasury][0].amount - 12345678) < 1e-6 && R.pending_by_wallet[treasury][0].usd != null && R.pending_by_wallet[treasury][1].usd == null && Object.keys(R.pending_by_wallet).filter(a => a !== treasury).every(a => Array.isArray(R.pending_by_wallet[a]) && R.pending_by_wallet[a].length === 0), R.pending_by_wallet[treasury]); }
