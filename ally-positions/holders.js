...
async function roar20Holders(t) {
  ...
  const roster = t.roar20 && t.roar20.known_owners || {};   // registry labels: { owner: label } or { owner: { label, kind, program } }
  // 1.4.0: a registry entry can say WHAT an owner is. A pool's authority (Raydium AMM v4's 5Q544…) is a keyless PDA the System Program
  //   owns, so the chain read calls it a "wallet" — ranking a liquidity pool at 73 % among wallets. The registry corrects it; its kind wins.
  holders.forEach(h => { const k = roster[h.owner]; if (!k) { h.label = null; return; } if (typeof k === 'string') { h.label = k; return; }
    h.label = k.label || null; if (k.kind) { h.kind = k.kind; h.owner_program = k.program || h.owner_program || null; h.kind_source = 'registry: tenants.json roar20.known_owners (' + (k.source || 'owner') + ')'; } });
  const by = (k) => holders.filter(h => h.kind === k);
...
async function roarWhales(t, nameOf) {
  ...
  // 1.2.0: every holder of a bank denom (ampROAR, a native LP token) — the path form first, the by_query form when the denom's slashes break it
  const denomOwners = async (denom) => { const owners = []; let key = null, pages = 0, byQuery = false;
    for (;;) { const kq = key ? `&pagination.key=${encodeURIComponent(key)}` : ''; let r = byQuery ? null : await lcd(`/cosmos/bank/v1beta1/denom_owners/${encodeURIComponent(denom)}?pagination.limit=1000` + kq); if (!r || !Array.isArray(r.denom_owners)) { byQuery = true; r = await lcd(`/cosmos/bank/v1beta1/denom_owners_by_query?denom=${encodeURIComponent(denom)}&pagination.limit=1000` + kq); }
      if (!r || !Array.isArray(r.denom_owners)) return { owners, fail: `denom_owners page ${pages + 1} failed (path and by_query)` };
      pages++; r.denom_owners.forEach(o => owners.push(o)); key = r.pagination && r.pagination.next_key; if (!key) break; if (pages > 200) return { owners, fail: 'denom_owners exceeded 200 pages' }; }
    return { owners, fail: null }; };
  ...
  if (st.amproar_denom && st.amproar_hub) { const hs = await smartOn(st.amproar_hub)({ state: {} }); const xr = hs && hs.exchange_rate != null ? Number(hs.exchange_rate) : null; const owners = []; let key = null, pages = 0, fail = null;
    let byQuery = false; for (;;) { const kq = key ? `&pagination.key=${encodeURIComponent(key)}` : ''; let r = byQuery ? null : await lcd(`/cosmos/bank/v1beta1/denom_owners/${encodeURIComponent(st.amproar_denom)}?pagination.limit=1000` + kq); if (!r || !Array.isArray(r.denom_owners)) { byQuery = true; r = await lcd(`/cosmos/bank/v1beta1/denom_owners_by_query?denom=${encodeURIComponent(st.amproar_denom)}&pagination.limit=1000` + kq); }   /* 1.2.0: a factory denom's slashes break the path form */ if (!r || !Array.isArray(r.denom_owners)) { fail = `denom_owners page ${pages + 1} failed`; break; } pages++; r.denom_owners.forEach(o => owners.push(o)); key = r.pagination && r.pagination.next_key; if (!key) break; if (pages > 200) { fail = 'denom_owners exceeded 200 pages'; break; } }
  ...
  { const pairs = Array.isArray(t.roar_pools) ? t.roar_pools : []; let any = false, fails = []; let sum = 0; const detail = [];
    for (const p of pairs) { const q = smartOn(p.address); const pool = await q({ pool: {} }); const pair = await q({ pair: {} }); const lpRaw = pair && (pair.liquidity_token || pair.liquidity_token_addr); const lpTok = lpRaw && typeof lpRaw === 'object' ? ((lpRaw.native_token && lpRaw.native_token.denom) || (lpRaw.token && lpRaw.token.contract_addr) || null) : lpRaw;   // 1.2.0: a SkeletonSwap (ex-White Whale) pair's LP is a tokenfactory denom, maybe given as an asset-info object if (!pool || !Array.isArray(pool.assets) || !lpTok) { fails.push(`${p.label}: pool/pair read failed`); continue; }
      ...
      if (/^factory\//.test(lpTok)) {   // 1.2.0: native LP — its holders are bank denom_owners, not a cw20 walk
        const { owners, fail: f2 } = await denomOwners(lpTok); if (f2) { fails.push(`${p.label}: ${f2}`); continue; }
        let psum = 0; for (const o of owners) { const sh = num(o.balance && o.balance.amount); if (sh > 0) { const roar = poolRoar * sh / total; const w = at(o.address); w.lp_plain = (w.lp_plain || 0) + roar; psum += roar; } }
        any = true; sum += psum; detail.push({ pair: p.label, address: p.address, lp_token: lpTok, lp_kind: 'native (bank denom_owners)', pool_roar: poolRoar, holders: owners.length, roar_attributed: psum }); continue; }
  ...
  // 1.2.0 CUSTODY — contracts whose ROAR belongs to others. The registry names them (the staking module, the amp hub, the registered pairs,
  // tenants.json whales.custody); the chain's own contract label names the rest (a pair, an incentives/generator contract). They leave the
  // ranking and every sum, and are published beside them with the reason and where (if anywhere) their ROAR is attributed instead.
  const pairSet = new Set((Array.isArray(t.roar_pools) ? t.roar_pools : []).map(p => p.address));
  const extra = (t.whales && Array.isArray(t.whales.custody)) ? t.whales.custody : [];
  const custodyOf = (w) => {
    if (st.roar_staking && w.address === st.roar_staking) return 'the staking module — holds everyone\'s staked ' + (info.symbol || 'ROAR') + ' (the Staked column attributes it to each staker)';
    if (st.amproar_hub && w.address === st.amproar_hub) return 'the ampROAR hub — its ROAR backs ampROAR' + (cols.amp && cols.amp.ok ? ' (the amp column attributes it to each holder)' : ' (the amp column was not read this run, so it is attributed to no one)');
    if (pairSet.has(w.address)) return 'a registered pair — its ROAR belongs to its LP holders (the Plain LP column attributes it)';
    const x = extra.find(e => (e.address || e) === w.address); if (x) return (x.reason || 'named custody in tenants.json whales.custody');
  ...
  for (const w of rows) { const why = custodyOf(w); if (why) { w.custody = why; custody.push(w); } else ranked.push(w); }
  ranked.forEach((w, i) => { w.rank = i + 1; }); custody.forEach((w, i) => { w.rank = null; w.custody_rank = i + 1; });
  const custodySet = new Set(custody.map(w => w.address));
  const totalSupply = H(info.total_supply); const all = [...W.values()].filter(w => !custodySet.has(w.address)); const sumCol = (k) => all.reduce((a, w) => a + (w[k] || 0), 0);
  return { product: {
    ...
    holder_count: ranked.length, wallets_seen: all.length, holders: ranked,
    custody: { count: custody.length, total: custody.reduce((a, w) => a + (w.total || 0), 0), note: 'contracts holding ROAR for others — not ranked, not in sums or concentration (1.2.0)', rows: custody },
    top25: ranked.slice(0, 25).map(w => ({ rank: w.rank, address: w.address, label: w.label, kind: w.kind, staked: w.staked, liquid: w.liquid, amp: w.amp, lp_amp: w.lp_amp, lp_plain: w.lp_plain, total: w.total })),
    sums: { staked: sumCol('staked'), liquid: sumCol('liquid'), amp: sumCol('amp'), lp_amp: sumCol('lp_amp'), lp_plain: sumCol('lp_plain'), total: all.reduce((a, w) => a + (w.total || 0), 0), note: 'over every holder seen except custody contracts, before the min_total_roar floor that trims the table' },
    concentration: { top1_pct: ranked[0] && totalSupply ? ranked[0].total / totalSupply * 100 : null, top10_pct: totalSupply ? ranked.slice(0, 10).reduce((a, w) => a + w.total, 0) / totalSupply * 100 : null, top25_pct: totalSupply ? ranked.slice(0, 25).reduce((a, w) => a + w.total, 0) / totalSupply * 100 : null, of: 'total_supply' },
